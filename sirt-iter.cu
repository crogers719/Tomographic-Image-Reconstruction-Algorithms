...
void Ultrasonic_Tomography(const string &fo_filename, int group_size, float target_epsilon, int max_iterations, float omega, float alpha, float beta)
{
	...
	for(int iter = 0; iter < max_iterations; iter++)
	{
		...
		for(int p = 0; p < NS; p += group_size)
		{
			...
			difference_signal<<<grid_diff_signal, threads_diff_signal>>>(dev_u, dev_g_bottom, dev_g_right, dev_g_top, dev_g_left, dev_rr_bottom, dev_rr_right, dev_rr_top, dev_rr_left, group);
		...
		float scale = omega * (alpha + beta * (iter+1));

		update_field<<<Grid_Size, Block_Size>>>(dev_f, dev_df, dev_f_minus_fo, dev_fo, scale, Ng);
