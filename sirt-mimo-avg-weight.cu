...
__global__ void get_normalized_weights(
		kernel_ptr<float> weight,
		kernel_ptr<float> const fg,
		kernel_ptr<float> const f_avg,
		int Ng)
{
...
__global__ void get_total_weight(
		kernel_ptr<float> total_w,
		kernel_ptr<float> avg_w,
		kernel_ptr<float> const weight, 
		int Ng)
{
	total_w(0) = 0.f;
	for (int g = 0; g < Ng; ++g) {
	...
	avg_w(0) = total_w(0) / Ng;
}
__global__ void update_weighted_average_field(
	kernel_ptr<float> f_avg,
	kernel_ptr<float> const fg,
	kernel_ptr<float> const weight,
	kernel_ptr<float> const total_w,
	int Ng)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int j = threadIdx.y + blockIdx.y * blockDim.y;
...
void Ultrasonic_Tomography(const string &fo_filename, int group_size, float target_epsilon, int max_iterations, int ti)
{
	...
	device_ptr<float> dev_f_avg(NX, NY);
	dev_f_avg.set(0.f);

	host_ptr<float> weight(Ng);
	device_ptr<float> dev_weight(Ng);
	...
	dim3 threads_weights(1, 1);
	dim3 grid_weights(
		grid_size(Ng, threads_weights.x));

	cerr << "group size:     " << group_size << "\n"
		 << "target epsilon: " << target_epsilon << "\n\n";

	device_ptr<float> dev_total_w(1);
	host_ptr<float> total_w(1);

	device_ptr<float> dev_avg_w(1);
	host_ptr<float> avg_w(1);
	...
	for(int iter = 0; iter < max_iterations; iter++)
	{
		...
		sum_intermediate_fields<<<grid_inter_fields, threads_inter_fields>>>(dev_f_avg, dev_fg, Ng);

		average_intermediate_fields<<<grid_field, threads_field>>>(dev_f_avg, Ng);

		get_normalized_weights<<<grid_weights, threads_weights>>>(dev_weight, dev_fg, dev_f_avg, Ng);

		get_total_weight<<<1,1>>>(dev_total_w, dev_avg_w, dev_weight, Ng);

		dev_f_avg.set(0.f);

		update_weighted_average_field<<<grid_inter_fields, threads_inter_fields>>>(dev_f_avg, dev_fg, dev_weight, dev_total_w, Ng);

		update_field<<<grid_field, threads_field>>>(dev_f, dev_f_minus_fo, dev_f_avg, dev_fo, dev_total_w, Ng, iter+1);
