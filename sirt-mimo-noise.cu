...
__global__ void noise_setup (kernel_ptr<curandState>);
...
__global__ void initial_signal(kernel_ptr<float> const,	kernel_ptr<float>, kernel_ptr<float>,	kernel_ptr<float>,kernel_ptr<float>, kernel_ptr<curandState>, int);
__global__ void difference_signal(kernel_ptr<float> const,kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const,	kernel_ptr<float>,kernel_ptr<float>,	kernel_ptr<float>, kernel_ptr<float>, kernel_ptr<curandState>, int);
...
void Ultrasonic_Tomography(int group_size, float target_epsilon, int max_iterations, int ti, float omega)
{
	...
	dim3 threads_noise(NX, 1, 1);
	dim3 grid_noise(
			grid_size(NX, threads_noise.x),
			grid_size(NT, threads_noise.y),
			grid_size(Ng, threads_noise.z));
	...
	initial_signal<<<grid_signal, threads_signal>>>(dev_u, dev_g_bottom, dev_g_right, dev_g_top, dev_g_left, dev_state, Ng);
	...
	for(int iter = 0; iter < max_iterations; iter++)
	{
		...
		difference_signal<<<grid_diff_signal, threads_diff_signal>>>(dev_u, dev_g_bottom, dev_g_right, dev_g_top, dev_g_left, dev_rr_bottom, dev_rr_right, dev_rr_top, dev_rr_left , dev_state, Ng);
	...
	int Ng)
{
	...
	if (i > 20 && i < 180 && k > 1 && k < NT && g < Ng) {
		...
		rr_bottom(i, k, g) = g_bottom(i, k, g) - u(i, 180, k, g)* noise;

		// store difference at time k of original signal
		// and current signal at top sensor row
		rr_top(i, k, g) = g_top(i, k, g) - u(i, 20, k, g) * noise;

		// store difference at time k of original signal
		// and current signal at right sensor column
		rr_right(i, k, g) = g_right(i, k, g) - u(180, i, k, g) * noise;

		// store difference at time k of original signal
		// and current signal at left sensor column
		rr_left(i, k, g) = g_left(i, k, g) - u(20, i, k, g) *noise ;
	}
}
