...
//Host Functions
void Ultrasonic_Tomography(const string &fo_filename, int group_size, int Np, float target_epsilon, int max_iterations, float alpha1, float beta1, float n1, float alpha2, float beta2, float n2);
...
int main(int argc, char **argv)
{
	...
	if (argc != 12) {
		cerr << "Usage: " << argv[0] << " <fo_filename> <group size> <parallel groups> <target epsilon> <max iterations> <alpha1> <beta1> <n1> <alpha2> <beta2> <n2>\n\n";
		exit(1);
	}
	...
	float alpha1 = stof(argv[6]) * 1000.f;
	float beta1 = stof(argv[7]) * 1000.f;
	float n1 = stof(argv[8]);
	float alpha2 = stof(argv[9]) * 1000.f;
	float beta2 = stof(argv[10]) * 1000.f;
	float n2 = stof(argv[11]);
	...
	Ultrasonic_Tomography(fo_filename, group_size, Np, target_epsilon, max_iterations, alpha1, beta1, n1, alpha2, beta2, n2);
	cudaDeviceReset();
}
...
void Ultrasonic_Tomography(const string &fo_filename, int group_size, int Np, float target_epsilon, int max_iterations, float alpha1, float beta1, float n1, float alpha2, float beta2, float n2)
{
	...
	while (!reached && !diverged && iter < max_iterations) {
		...
		float scale =
			odd ?
				alpha1 + (beta1 / pow(iter, n1)) :
				alpha2 - (beta2 / pow(iter, n2));
