...
//Host Functions
void Ultrasonic_Tomography(const string&, int, int, float, int, float, float, float, float);
...
int main(int argc, char **argv)
{
	...
	if (argc != 10) {
		cerr << "Usage: " << argv[0] << " <fo_filename> <group size> <parallel groups> <target epsilon> <max iterations> <omega> <alpha> <beta> <gamma>\n\n";
		exit(1);
	}
	...
	float omega = stof(argv[6]);
	float alpha = stof(argv[7]);
	float beta = stof(argv[8]);
	...
	Ultrasonic_Tomography(fo_filename, group_size, Np, target_epsilon, max_iterations, omega, alpha, beta, gamma);
	cudaDeviceReset();
}
...
void Ultrasonic_Tomography(const string &fo_filename, int group_size, int Np, float target_epsilon, int max_iterations, float omega, float alpha, float beta, float gamma)
{
	...
	while (!reached && !diverged && iter < max_iterations) {
		...
		float scale = omega * (alpha + (beta / pow(iter, gamma)));
