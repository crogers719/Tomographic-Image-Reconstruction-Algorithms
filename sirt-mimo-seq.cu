...
__global__ void weights_differential1(kernel_ptr<float> , kernel_ptr<float> const, kernel_ptr<float> const, int);
__global__ void weights_differential2(kernel_ptr<float>, kernel_ptr<float>, kernel_ptr<float> const, int);
__global__ void average_differential(kernel_ptr<float>, kernel_ptr<float>const, kernel_ptr<float> const, kernel_ptr <float> const, int);


__global__ void art_propagation (int, int, int, int, kernel_ptr<float> const, kernel_ptr<float>, int);
__global__ void art_propagation_at_corners (kernel_ptr<float>);
__global__ void art_difference_signal (kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float>, kernel_ptr<float>, kernel_ptr<float>, kernel_ptr<float>, int);
__global__ void art_backpropagation1(kernel_ptr<float>, kernel_ptr<float> const, int);
__global__ void art_backpropagation2(kernel_ptr<float>, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const, int);
__global__ void art_laplace(kernel_ptr<float>const, kernel_ptr<float>);
__global__ void art_laplace_corners (kernel_ptr<float> const, kernel_ptr<float>);
__global__ void art_update_differential(kernel_ptr<float>, kernel_ptr<float> const, kernel_ptr<float> const, kernel_ptr<float> const);
__global__ void art_update_field (kernel_ptr<float>, kernel_ptr<float> const, kernel_ptr<float>, kernel_ptr<float> const, float);
...
/*************************************************************************ART*********************************************************************/
	...
	dim3 Grid_Size(grid_size(NX, 16), grid_size(NY, 16));	


	dim3 threads_propagation_art(NX, 1, 1);
	dim3 grid_propagation_art(
			grid_size(NX, threads_propagation_art.x),
			grid_size(NY, threads_propagation_art.y));

	dim3 threads_diff_signal_art(NX, 1);
	dim3 grid_diff_signal_art(
			grid_size(NX, threads_diff_signal_art.x),
			grid_size(NT, threads_diff_signal_art.y));

	dim3 threads_backpropagation1_art(NX, 1, 1);
	dim3 grid_backpropagation1_art(
			grid_size(NX, threads_backpropagation1_art.x),
			grid_size(NY, threads_backpropagation1_art.y));

	dim3 threads_laplace_art(96, 2, 1);
	dim3 grid_laplace_art(
			grid_size(NX, threads_laplace_art.x),
			grid_size(NY, threads_laplace_art.y),
			grid_size(NT, threads_laplace_art.z));

	dim3 threads_differential_art(96, 2, 1);
	dim3 grid_differential_art(
			grid_size(NX, threads_differential_art.x),
			grid_size(NY, threads_differential_art.y),
			grid_size(NT, threads_differential_art.z));
...
for(int iter = 0; iter < INITIAL_ITERATIONS; iter++)
	{
		...
		for(int p = 0; p < NS; p += group_size)
		{
			...
			{
				art_propagation<<<grid_propagation_art, threads_propagation_art>>>(jp1, jp2, ip1, ip2, dev_f, dev_u, k);
			}

			// Four corners

			art_propagation_at_corners<<<NT, 1>>>(dev_u);
			art_difference_signal<<<grid_diff_signal_art, threads_diff_signal_art>>>(dev_u, dev_g_bottom, dev_g_right, dev_g_top, dev_g_left, dev_rr_bottom, dev_rr_right, dev_rr_top, dev_rr_left, p / group_size);

			dev_z.set(0.f);

			for (int k = NT - 2; k > 0; k--)
			{
				art_backpropagation1<<<grid_backpropagation1_art, threads_backpropagation1_art>>>(dev_z, dev_f, k);
				art_backpropagation2<<<NX, 1>>>(dev_z, dev_rr_bottom, dev_rr_right, dev_rr_top, dev_rr_left, k);
			}

			art_laplace<<<grid_laplace_art, threads_laplace_art>>>(dev_u, dev_Lu);
			art_laplace_corners<<<NT, 1>>>(dev_u, dev_Lu);

			dev_df.set(0.f);
			art_update_differential<<<grid_differential_art, threads_differential_art>>>(dev_df, dev_z, dev_Lu, dev_f);

			art_update_field<<<Grid_Size, Block_Size>>>(dev_f, dev_df, dev_f_minus_fo, dev_fo, scale);
		}
...
/*************************************************************************************************SIRT***********************************************************/
	...
	for (int iter =INITIAL_ITERATIONS; iter<max_iterations; iter++){
			if (diverged==true){
				break;
			}
...
__global__ void art_propagation(
...
__global__ void art_propagation_at_corners(kernel_ptr<float> u)
{
	int k = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void art_difference_signal(
		kernel_ptr<float> const u,
		kernel_ptr<float> const g_bottom,
		kernel_ptr<float> const g_right,
		kernel_ptr<float> const g_top,
...
__global__ void art_backpropagation1(
		kernel_ptr<float> z,
		kernel_ptr<float> const f,
		int k)
{
...
__global__ void art_backpropagation2(
		kernel_ptr<float> z,
		kernel_ptr<float> const rr_bottom,
		kernel_ptr<float> const rr_right,
		kernel_ptr<float> const rr_top,
...
__global__ void art_laplace_corners(kernel_ptr<float> const u, kernel_ptr<float> Lu)
{
	int k = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void art_update_differential(
		kernel_ptr<float> df,
		kernel_ptr<float> const z,
		kernel_ptr<float> const Lu,
		kernel_ptr<float> const f)
...
__global__ void art_update_field(
		kernel_ptr<float> f,
		kernel_ptr<float> const df,
		kernel_ptr<float> f_minus_fo,
		kernel_ptr<float> const fo,
