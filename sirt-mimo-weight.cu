...
template <typename T> 
__host__ __device__ void minmax(T &a, T &b);
...
__global__ void update_intermediate_fields (kernel_ptr<float>, kernel_ptr<float> const, kernel_ptr<float> const, float, float, float, int, int);
__global__ void sum_intermediate_fields (kernel_ptr<float>, kernel_ptr <float> const, int);
__global__ void average_intermediate_fields (kernel_ptr<float>, kernel_ptr<float>, kernel_ptr<float> const, kernel_ptr<float>, float);
...
int main(int argc, char **argv)
{
	...
	if (argc != 8) {
		cerr << "Usage: " << argv[0] << " <fo_filename> <sensor group size> <target epsilon> <max iterations> <alpha> <beta> <omega>\n\n";
		exit(1);
	}
	...
	Ultrasonic_Tomography(fo_filename, group_size, target_epsilon, max_iterations, alpha, beta, omega);
	cudaDeviceReset();
}
...
void Ultrasonic_Tomography(const string &fo_filename, int group_size, float target_epsilon, int max_iterations, float alpha, float beta, float omega)
{
	...
	ofstream convergence_file("sirt_weight_convergence.txt");
	...
	for(int iter = 0; iter < max_iterations; iter++)
	{
		...
		update_intermediate_fields<<<grid_inter_fields, threads_inter_fields>>>(dev_fg, dev_f, dev_df, alpha, beta, omega, iter+1, Ng);
		...
		int iteration, int Ng)
{
	...
	if (i < NX && j < NY && g < Ng)
	{
		...
		float scale = in_sensor_field ? omega * (alpha + (beta / iteration)) : 0.f;

		fg(i, j, g) = f(i, j) + scale * df(i, j, g);
	}
}
