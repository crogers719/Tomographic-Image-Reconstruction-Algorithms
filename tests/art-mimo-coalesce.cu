...
__global__ void field_setup(const float *x, const float *y, float *fo)
{
	...
	if((i < NX) && (j < NY))				//in range
	{
		...
		if ((powf(x[i] - rc * cos(-30 * (3.14159265f / 180)), 2) + powf(y[j] - rc * sin(30 * (3.14159265f / 180)), 2) <= powf(rp, 2)) ||
				(powf(x[i] + rc * cos(-30 * (3.14159265f / 180)), 2) + powf(y[j] - rc * sin(30 * (3.14159265f / 180)), 2) <= powf(rp, 2)) ||
				(powf(x[i], 2) + powf(y[j] + rc, 2) <= powf(rp, 2)) )
		{
			value = sp;
		}
...
inline __device__ float wave(float v, float r, float s, float t)
{
    return (v * v * dt * dt * cosf(omegac * t) * expf(-(t * t) / (2.f * tao * tao)));
}
...
__global__ void propagation(int jp1, int jp2, int ip1, int ip2, const float *f, float *u, int k)
{
	...
	//In bounds and not a corner
	if( ((i < NX) && (j < NY)) && 
		(!((i == 0 && j == 0) || (i == 0 && j == NY - 1) || (i == NX - 1 && j == 0) || (i == NX - 1 && j == NY - 1))))	 
	{				
        ...
        // left boundary
      else if ((i == 0) && (j > 0) && (j < (NY - 1)))
			{
					val =
					    ...
					    2.f * r * (1.f + r) * u[(i + 1) + NX * j + NX * NY * k] -
					    r * r * u[(i + 2) + NX * j + NX * NY * k] +
					    (2.f * r - 1.f) * u[i + NX * j + NX * NY * (k - 1)] -
					    2.f * r * u[(i + 1) +  NX * j + NX * NY * (k - 1)];
			}
...
__global__ void propagation_at_corners(float *u)
{
	...
	u[(NX - 1) + (NX *NY * k)] =
	    1.f / 2.f * (u[(NX - 2) + (NX *NY * k)] + u[(NX - 1) + NX + (NX *NY * k)]);

	u[(NY - 1) * NX + (NX *NY * k)] =
	    1.f / 2.f * (u[(NY - 2) * NX + (NX *NY * k)] + u[1 +(NY - 1) * NX + (NX *NY * k)]);

	u[(NX - 1) + (NY - 1) * NX + (NX *NY * k)] =
	    1.f / 2.f * (u[(NX - 2) + (NY - 1) * NX + (NX *NY * k)] + u[(NX - 1) + (NY - 2) * NX + (NX *NY * k)]);
...
__global__ void backpropagation1(float *z, const float *f, int k)
{
	...
	if((i >= 1) && (i < (NX - 1)) && (j >= 1) && (j < (NY - 1)))		//inside
	{
		z[i + NX * j + NX * NY * k] =
		    1500.f * 1500.f * (dt * dt) *
...
__global__ void init_differential(float *df, const float *z, const float *Lu, const float *f)
{
	...
	if((i < NX) && (j < NY))
	{
		...
		df[offset] = z[offset + NX * NY] * Lu[offset + NX * NY] / (1.f + f[offset]);
	}
}
