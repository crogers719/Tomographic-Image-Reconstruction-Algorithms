...
void Ultrasonic_Tomography(int group_size, float target_epsilon, int max_iterations, int ti, float omega, int dimx, int dimy, int dimz);
...
void IO_Files(
	host_ptr<float> const fo,
	host_ptr<float> const f)
{
	int i = 0, j = 0;
...
{
	if (argc != 8) {
		cerr << "Usage: " << argv[0] << " <sensor group size> <target epsilon> <max iterations> <omega> <dimx> <dimy> <dimz>\n\n";
		exit(1);
	}
	...
	int dimx = stoi(argv[5]);
	int dimy = stoi(argv[6]);
	int dimz = stoi(argv[7]);
	...
	Ultrasonic_Tomography(group_size, target_epsilon, max_iterations, ti, omega, dimx, dimy, dimz);
	cudaDeviceReset();

	tf = clock();
	cerr << "time = " << setprecision(8) << (float)(tf - ti) / CLOCKS_PER_SEC << "s\n";
}
...
void Ultrasonic_Tomography(int group_size, float target_epsilon, int max_iterations, int ti, float omega, int dimx, int dimy, int dimz)
{
	...
	dim3 threads_propagation(32, 2, 2);
	...
	dim3 threads_backpropagation1(64, 2, 1);
