...
void IO_Files(float const *x, float const *y,
...
template <int Nx, int Ny>
float norm(Get2d<float, Nx> const A)
{
	float sum = 0;
...
__global__ void propagation_at_corners(Get4d<float, NX, NY, NT> u, int g)
{
...
__global__ void difference_signal(
		Get4d<float, NX, NY, NT> const u,
		Get3d<float, NX, NT> const g_bottom,
		Get3d<float, NX, NT> const g_right,
		Get3d<float, NX, NT> const g_top,
		Get3d<float, NX, NT> const g_left,
		Get3d<float, NX, NT> rr_bottom,
		Get3d<float, NX, NT> rr_right,
		Get3d<float, NX, NT> rr_top,
		Get3d<float, NX, NT> rr_left,
		int g)
{
	int i = threadIdx.x;
	int k = blockIdx.x;
...
__global__ void backpropagation2(
		Get4d<float, NX, NY, NT+1> z,
		Get3d<float, NX, NT> const rr_bottom,
		Get3d<float, NX, NT> const rr_right,
		Get3d<float, NX, NT> const rr_top,
		Get3d<float, NX, NT> const rr_left,
		int k, int g)
{
	int i = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void update_differential(
		Get2d<float, NX> df,
		const Get4d<float, NX, NY, NT+1> z,
		Get4d<float, NX, NY, NT> const Lu,
		Get2d<float, NX> const f,
		int g)
{
	// Map from threadIdx / BlockIdx to pixel position
